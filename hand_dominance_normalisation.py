...
class HandDominanceNormaliser:
    def swapToRightDominant(self, filepath, show_logs=False):
        ...
        if show_logs:
            print(
                f"[swapToRightDominant] {filepath} — swapped in place, swapped_dominance=True")

    def matchSwappedCorpus(
            self,
            preswapped_corpus,
            target_corpus,
            show_logs=False):
        ...
        for index, preswapped_item, in enumerate(preswapped_items):
            ...
            if preswapped_data['metadata'].get("swapped_dominance"):
...
class DominanceLabellingWindow(QMainWindow):
    ...
    def __init__(self, root_dir=None):
        ...
        self.playback.frame_changed.connect(
            lambda i: self.graph_view.update_frame(
                self.data_model.get_frame(i)))
    ...
    def _initialise_queue(self):
        ...
        if not self.file_queue:
            QMessageBox.information(
                self, "Done", "All files have already been labelled!")
            return
...
if __name__ == "__main__":
    ...
    dom_hander.swapToRightDominant(
        filepath=r"C:\Users\Oscar Strong\Documents\GitHub\BSL-keypoint-processing\keypoints_V2\O\89abb1f9-9daf-48bd-96cd-b0c4afdfa311.json",
        show_logs=True)
    dom_hander.swapToRightDominant(
        filepath=r"C:\Users\Oscar Strong\Documents\GitHub\BSL-keypoint-processing\keypoints_V2\U\89abb1f9-9daf-48bd-96cd-b0c4afdfa311.json",
        show_logs=True)
    '''
