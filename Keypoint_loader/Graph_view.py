...
class GraphView(pg.GraphicsLayoutWidget):
    ...
    def __init__(self):
        ...
        # {(filename, cluster, point_idx): metadata_dict}
        self.point_metadata = {}
        # {(filename, cluster): numpy array of points}
        self.scatter_points = {}
        # {(filename, palm_type): ScatterPlotItem}
        self.palm_scatter_items = {}
        # {(filename, palm_type, point_idx): metadata_dict}
        self.palm_metadata = {}
            ...
            for idx, point in enumerate(points_array):
                ...
                if distance < hover_distance:
                    ...
                    if meta_key in self.point_metadata:
                        ...
                        if new_hover_key != self.current_hover_key or (
                                now - self.last_tooltip_refresh) > self.tooltip_refresh_interval:
                            QToolTip.showText(
                                QCursor.pos(), tooltip_text, self)
    ...
    def update_frame(self, frame):
        ...
        keys_to_remove = [
            k for k in self.scatter_items if k[0] not in points_by_file]
        for k in keys_to_remove:
            ...
            meta_keys_to_remove = [
                mk for mk in self.point_metadata if mk[0] == k[0] and mk[1] == k[1]]
        ...
        palm_keys_to_remove = [
            k for k in self.palm_scatter_items if k[0] not in palms_by_file]
        for k in palm_keys_to_remove:
            ...
            # also remove metadata for removed items
            meta_keys_to_remove = [
                mk for mk in self.palm_metadata if mk[0] == k[0] and mk[1] == k[1]]
            for mk in meta_keys_to_remove:
        ...
        if self.palm_mode in ['real', 'kalman', 'cubic_spline', 'pchip']:
            for filename, data in palms_by_file.items():
                ...
                palm_data_mapping = {
                    'real': (
                        'real_palms', 'palm_metadata', 'o'), 'kalman': (
                        'kalman_palms', 'kalman_metadata', 's'), 'cubic_spline': (
                        'cubic_spline_palms', 'cubic_spline_metadata', '^'), 'pchip': (
                        'pchip_spline_palms', 'pchip_spline_metadata', 'd')}
                ...
                if key not in self.palm_scatter_items:
                    scatter = pg.ScatterPlotItem(
                        size=15, symbol=marker_symbol, pen=color, brush=None)
                    self.plot.addItem(scatter)
