...
class DataModel:
    ...
    def load_from_json(self, json_path, add=True, load_palms=True):
        ...
        if not self.frames:
            ...
            if num_frames > len(self.frames):
                self.frames.extend(
                    [{} for _ in range(num_frames - len(self.frames))])
        ...
        if load_palms:
            try:
                ...
                print(
                    f"Warning: Could not load palm centers from {filename}: {e}")
        ...
        try:
            ...
            for frame_idx, palms in enumerate(palm_centers):
                ...
                if filename not in self.frames[frame_idx]["palms_by_file"]:
                    self.frames[frame_idx]["palms_by_file"][filename] = {
                        "color": color}
                ...
                for side in ['left', 'right']:
                    if side in palms and palms[side] != [None, None]:
                        ...
                        palm_metadata.append(
                            {'side': side, 'x': palms[side][0], 'y': palms[side][1]})

                # Merge with existing data, preserving color and estimated
                # palms if they exist
                self.frames[frame_idx]["palms_by_file"][filename]["real_palms"] = self._to_2d(
                    palm_points)
    ...
    def _load_estimated_palm_centers(self, filepath, filename, color):
        ...
        try:
            ...
            for frame_idx, palms in enumerate(filled_palms):
                ...
                # merge with existing data, preserving color and real palms if
                # they exist
                self.frames[frame_idx]["palms_by_file"][filename]["estimated_palms"] = self._to_2d(
                    palm_points)
    ...
    def load_palm_centers_by_type(self, filepath, filename, palm_type, color):
        ...
        if palm_type == 'real':
            ...
            interpolator = CubicSplineKeyPointInterpolator(
                filepath, method='cubic')
            self._load_spline_palm_centers(
                interpolator, filename, color, 'cubic')
        elif palm_type == 'pchip':
            interpolator = CubicSplineKeyPointInterpolator(
                filepath, method='pchip')
            self._load_spline_palm_centers(
                interpolator, filename, color, 'pchip')
    ...
    def _load_kalman_palm_centers(self, estimator, filename, color):
        ...
        try:
            ...
            for frame_idx, palms in enumerate(filled_palms):
                ...
                self.frames[frame_idx]["palms_by_file"][filename]["kalman_palms"] = self._to_2d(
                    palm_points)
    ...
    def _load_spline_palm_centers(
        ...
        try:
            ...
            for frame_idx, palms in enumerate(filled_palms):
                ...
                self.frames[frame_idx]["palms_by_file"][filename][method_key] = self._to_2d(
                    palm_points)
