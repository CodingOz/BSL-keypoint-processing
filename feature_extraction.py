...
class FeatureExraction:
    ...
    def extract_angle_features(self, json_obj, include_labels=False):
        ...
        for frame_idx, frame in enumerate(frames):
            ...
            if (left_landmarks is not None and right_landmarks is not None and 
                ...
                for left_idx, right_idx in inter_hand_pairs:
                    angle = self._calculate_angle(left_landmarks[left_idx], 
                                                  left_landmarks[0],  # left wrist as reference
            ...
            if include_labels:
                frame_with_labels = [[angle, frame_idx] for angle in frame_angles]
    ...
    def extract_feature_set_1_combined(self, json_obj):
        ...
        '''
        proximity = self.extract_proximity_features(json_obj, include_labels=False)
            ...
            if left_landmarks is not None and right_landmarks is not None:
                for i, idx1 in enumerate(key_indices):
                    for j, idx2 in enumerate(key_indices):
                        if i < j:
                            # Left hand distances
                            dist = np.linalg.norm(left_landmarks[idx1] - left_landmarks[idx2])
                ...
                for i, idx1 in enumerate(key_indices):
                    for j, idx2 in enumerate(key_indices):
                        if i < j:
                            ...
                            dist = np.linalg.norm(right_landmarks[idx1] - right_landmarks[idx2])
                            frame_features.append(float(dist))
        ...
        for frame in frames:
            ...
            if left_landmarks is not None and right_landmarks is not None:
                ...
                for i, idx1 in enumerate(key_indices):
                    for j, idx2 in enumerate(key_indices):
                        if i < j:
                            dist_l = np.linalg.norm(left_landmarks[idx1] - left_landmarks[idx2])
                            dist_r = np.linalg.norm(right_landmarks[idx1] - right_landmarks[idx2])
                            dist_inter = np.linalg.norm(left_landmarks[idx1] - right_landmarks[idx2])
                            frame_features.extend([float(dist_l), float(dist_r), float(dist_inter)])
                ...
                for i, idx1 in enumerate(key_indices):
                    for j, idx2 in enumerate(key_indices):
                        for k, idx3 in enumerate(key_indices):
                            if i < j < k:
                                angle_l = self._calculate_angle(left_landmarks[idx1], 
                                                               left_landmarks[idx2], 
                                                               left_landmarks[idx3])
                                angle_r = self._calculate_angle(right_landmarks[idx1], 
                                                               right_landmarks[idx2], 
                                                               right_landmarks[idx3])
                                frame_features.extend([float(angle_l), float(angle_r)])
    ...
    def extract_feature_set_6_extreme_minimal(self, json_obj):
        ...
        for frame in frames:
            ...
            if left_landmarks is not None and right_landmarks is not None:
                # Distances from left index finger (8) to all right hand landmarks
                for i in range(len(right_landmarks)):
                    dist = np.linalg.norm(left_landmarks[8] - right_landmarks[i])
                    frame_features.append(float(dist))
                
                # Distances from left palm (0) to all right hand landmarks
                for i in range(len(right_landmarks)):
                    dist = np.linalg.norm(left_landmarks[0] - right_landmarks[i])
                    frame_features.append(float(dist))
                
                # Distances from right index finger (8) to all left hand landmarks
                for i in range(len(left_landmarks)):
                    dist = np.linalg.norm(right_landmarks[8] - left_landmarks[i])
                    frame_features.append(float(dist))
                
                # Distances from right palm (0) to all left hand landmarks
                for i in range(len(left_landmarks)):
                    dist = np.linalg.norm(right_landmarks[0] - left_landmarks[i])
                    frame_features.append(float(dist))
                
                # Distance between left and right pinkie fingertips (orientation context)
                pinkie_distance = np.linalg.norm(left_landmarks[20] - right_landmarks[20])
    ...
    def extract_all_feature_set_2(self):
        ...
        features = []
        for json_obj in self.corpus:
            features.append(self.extract_feature_set_2_proximity_only(json_obj))
        return features
    ...
    def extract_all_feature_set_3(self):
        ...
        features = []
        for json_obj in self.corpus:
            features.append(self.extract_feature_set_3_tips_and_palms_with_angles(json_obj))
        return features
    ...
    def extract_all_feature_set_4(self):
        ...
        features = []
        for json_obj in self.corpus:
            features.append(self.extract_feature_set_4_tips_and_palms_with_angles(json_obj))
        return features
    ...
    def extract_all_feature_set_5(self):
        ...
        features = []
        for json_obj in self.corpus:
            features.append(self.extract_feature_set_5_tips_and_palms_distances_only(json_obj))
        return features
