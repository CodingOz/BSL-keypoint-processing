...
def post_error_detection_pipeline(corpus_container_path, show_logs=False):
    ...
    keypoint_interpolator.estimateHandsEndsCorpusGenerator(level1a, level1b, show_logs=show_logs)
    
    print("level 1 processing done\n\n")
    
    # level 2: temporal cropping
    temporal_cropper = TemporalCropping()
    temporal_cropper.crop_single_handed_frames_in_corpus(level1b, level2a, show_logs=show_logs)
    temporal_cropper.crop_hands_resting_together_in_corpus(level2a, level2b, show_logs=show_logs)
    temporal_cropper.crop_to_stroke_phase_in_corpus(level2b, level2c, show_logs=show_logs)
    
    
    # level 3: temporal normalisation
    temporal_normaliser = TemporalNormalisor()
    temporal_normaliser.NormaliseCorpus(level2c, level3, frame_num=20, show_logs=show_logs)
...
if __name__ == "__main__":
    post_error_detection_pipeline(r"C:\Users\Oscar Strong\Documents\GitHub\BSL-keypoint-processing\Testing_post_error_ditection_pipeline")