...
class AnomalyDetection():
    ...
    def movementAnomalies(self,
        ...
        elif self.movement_flags is None:
    ...
    def positionAnomalies(self,
        ...
        if threshold:
            return self.validator.flagAbnormalDistances(
                outlier_boundry=threshold)
        elif self.posision_flags is None:
            self.posision_flags = self.validator.flagAbnormalDistances()
    ...
    def filledMovementAnomalies(self,
        ...
        interpolated = {
            'left': {
                i for i, f in enumerate(estimation_flags) if f.get('left')}, 'right': {
                i for i, f in enumerate(estimation_flags) if f.get('right')}, }
        ...
        for indx, side_flags in enumerate(flags):
            ...
            clean_flags = sorted(
                f for f in side_flags if f not in interpolated[side])
            ...
            # trim outer frames of each contiguous run — those are clean
            # boundary frames
            trimmed = []
            run_start = 0
    ...
    def filledMovementAnomaliesByPercentile(self, percentile, gap_size=5):
        ...
        flags = self.validator.findMovementRelativeByPercentile(
    ...
    def filledMovementAnomaliesByStdDev(self, num_std_dev, gap_size=5):
        ...
        flags = self.validator.findMovementRelativeByStdDev(
    ...
    def handOrderingAnomalysByPalmCenterUsingNeighbourFilling(self,
        ...
        """
        violations = self.validator.findHandOrderingByPalmCenterUsingNeighbourFilling(
            margin=margin)
    ...
    def OrderingByPalmsWithNeighbourFillingAndFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)

        posision = self.handOrderingAnomalysByPalmCenterUsingNeighbourFilling(
            margin=margin)
    ...
    def OrderingByWristsWithNeighbourFillingAndFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByWristUsingNeighbourFilling(
            margin=margin)
        left = set(movement[0]) & set(posision[0])
        right = set(movement[1]) & set(posision[1])
        return list(sorted(left)), list(sorted(right))

    def OrderingByExtremesWithNeighbourFillingAndFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByExtremesUsingNeighbourFilling(
            margin=margin)
        left = set(movement[0]) & set(posision[0])
        right = set(movement[1]) & set(posision[1])
        return list(sorted(left)), list(sorted(right))

    def OrderingByPalmsWithInterpolationAndFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByPalmCenterUsingInterpolation(
            margin=margin)
        left = set(movement[0]) & set(posision[0])
        right = set(movement[1]) & set(posision[1])
        return list(sorted(left)), list(sorted(right))

    def OrderingByWristsWithInterpolationAndFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByWristUsingInterpolation(
            margin=margin)
        left = set(movement[0]) & set(posision[0])
        right = set(movement[1]) & set(posision[1])
        return list(sorted(left)), list(sorted(right))

    def OrderingByExtremesWithInterpolationAndFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByExtremesUsingInterpolation(
            margin=margin)
        left = set(movement[0]) & set(posision[0])
        right = set(movement[1]) & set(posision[1])
        return list(sorted(left)), list(sorted(right))

    def OrderingByPalmsWithNeighbourFillingOrFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
    ...
    def OrderingByWristsWithNeighbourFillingOrFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByWristUsingNeighbourFilling(
    ...
    def OrderingByExtremesWithNeighbourFillingOrFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByExtremesUsingNeighbourFilling(
    ...
    def OrderingByPalmsWithInterpolationOrFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByPalmCenterUsingInterpolation(
    ...
    def OrderingByWristsWithInterpolationOrFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByWristUsingInterpolation(
    ...
    def OrderingByExtremesWithInterpolationOrFilledMovmentByStdDevIntersection(
            self, position_threshold=-0.15, num_std_dev=2.0, gap_size=5, margin=0.0):
        movement = self.filledMovementAnomalysByStdDev(num_std_dev=num_std_dev,
                                                       gap_size=gap_size)
        posision = self.handOrderingAnomalysByExtremesUsingInterpolation(
    ...
    def AccelerationAnomalys(
            self,
            threshold=0.15,
            inclusive=True,
            interpolate_missing=False):
        result = self.validator.findAccelerationClusters(
            margin=threshold, inclusive=inclusive, interpolate_missing=interpolate_missing)
    ...
    def positionAndFilledMovementAndAccelerationAnomalies(
            self,
    ...
    def findAppearanceDisappearanceSwaps(
            self,
            max_gap=2,
        ...
        for i in range(1, len(palms)):
            for appearing_side, disappearing_side in [
                ...
                if dist < distance_threshold:
                    ...
                    if show_logs:
                        print(
                            f"Frame {i}: {appearing_side} appeared at "
        ...
        # Return as tuple (left_flags, right_flags) — violations affect both
        # hands equally
        return violations, violations
