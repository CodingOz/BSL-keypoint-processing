...
# aims to help collect a random but representative corpus of 33 valid sign keypoints
class SubcorpusSelector:
    ...
    def selectRandomSigns(self, sign, n=33):
        ...
        if len(submissions) < n:
            print(f"Warning: Only {len(submissions)} submissions available for sign '{sign}'. Returning all.")
            return submissions
        return random.sample(submissions, n)
    
    def extractSubmissionId(self, filename):
        # Extract submission ID from filename (e.g., "1be98b34-0edc-41ee-871c-e592c0b4198f.json" -> "1be98b34-0edc-41ee-871c-e592c0b4198f")
    ...
    def checkDuplicateSubmissions(self, selected_dict):
        ...
        # Returns a dict with duplicate IDs as keys and list of signs they appear in as values
        submission_to_signs = {}
        ...
        duplicates = {sub_id: signs for sub_id, signs in submission_to_signs.items() if len(signs) > 1}
        return duplicates
    ...
    def selectRandomSignsFromAll(self, signs=[], n_submissions=3):
        ...
        all_signs = [d for d in os.listdir(self.corpus) if os.path.isdir(os.path.join(self.corpus, d))]
        if signs:
            selected = [sign for sign in all_signs if sign in signs]
        else:
            selected = random.sample(all_signs, min(n_submissions, len(all_signs)))
        
        # sort selected signs by number of submissions (ascending)
        sign_submission_counts = [(sign, len(self.listSigns(sign))) for sign in selected]
        ...
        for sign in sorted_signs:
            ...
            submissions_with_ids = [(f[:-5], f) for f in submissions if f.endswith('.json')]
            ...
            if len(selected_for_sign) < n_submissions:
                print(f"Warning: Only {len(selected_for_sign)} unique submissions available for sign '{sign}' without duplicates.")
        ...
        for sign, submissions in self.selected_signs.items():
            if signId in submissions:
                ...
                if new_selection:
                    ...
                    print(f"No more valid submissions available for sign '{sign}' to replace '{signId}'.")
                break
        return new_selection
    
    
selector = SubcorpusSelector(r"C:\Users\Oscar Strong\Desktop\finalProgect\KeypointCorpus_unprocessed")

# letter signs only
print(selector.selectRandomSignsFromAll(signs=['A', 'E', 'I', 'O', 'U', 'B', 'S', 'N', 'P', 'T', 'J'], n_submissions=3))