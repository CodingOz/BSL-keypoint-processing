...
class LayeredGaussianDistribution:
    ...
    def sample_region(self, total_frames: int, hole_size: int,
        ...
        if sign_data is not None:
            ...
            frames_with_hands = {
                i for i, f in enumerate(frames_list) if f.get(
                    'hands', {}).get('left') or f.get(
                    'hands', {}).get('right')}
        else:
        ...
        # fallback: return the full range and let the outer loop find the best
        # spot
...
class CorpusDegradator:
    ...
    def __init__(self, base_corpus_path: str, output_corpus_path: str,
        ...
        if not os.path.isdir(base_corpus_path):
            raise ValueError(
                f"Base corpus path '{base_corpus_path}' does not exist")
    ...
    def _compute_statistics(self, degraded_files: List[Dict]) -> Dict:
        ...
        for f in successful:
            for size in f['pattern']['swap_sizes']:
                swap_size_distribution[size] = swap_size_distribution.get(
                    size, 0) + 1
    ...
    def _sample_region_gaussian(self,
                                total_frames: int,
                                swap_size: int,
                                sign_data: dict = None) -> tuple[int,
                                                                 int]:
        """Existing Gaussian-based placement — delegates to LayeredGaussianDistribution."""
        return self.gaussian_dist.sample_region(
            total_frames, swap_size, sign_data=sign_data)

    def _sample_region_uniform(self,
                               total_frames: int,
                               swap_size: int,
                               sign_data: dict = None) -> tuple[int,
                                                                int]:
    ...
    def _apply_with_sampler(
        ...
        for idx, (filepath, pattern) in enumerate(
                zip(self.corpus_files, patterns)):
            print(
                f"\n[{idx+1}/{len(self.corpus_files)}] {os.path.basename(filepath)}"
                f"  ({sampler_label})")
            print(
                f"  Pattern: {pattern.num_swaps} swaps, sizes={pattern.swap_sizes}, types={pattern.swap_types}")
            ...
            try:
                ...
                for swap_idx, swap_size in enumerate(pattern.swap_sizes):
                    ...
                    # pre-compute which frames have hand data — do this once
                    # per file outside the loop
                    frames_with_hands = {
                        i for i, f in enumerate(frames_list) if f.get(
                            'hands', {}).get('left') or f.get(
                            'hands', {}).get('right')}
                    ...
                    for attempt in range(200):
                        ...
                        for start in range(
                            candidate_area[0], min(
                                candidate_area[1], total_frames - swap_size + 1)):
                    ...
                    if best_start is None or best_coverage == 0:
                        print(
                            f"    Skipping swap {swap_idx+1} — no frames with hand data available")
                        continue

                    if best_coverage < swap_size * 0.5:
                        print(
                            f"    Warning: swap {swap_idx+1} has only {best_coverage}/{swap_size} "
                            f"frames with hand data")

                    # pass a tight window so frameSwap is forced to use
                    # best_start
        ...
        total_actual_swaps = sum(len(f.get('swap_details', []))
                                 for f in successful)
        total_expected_swaps = sum(p.num_swaps for p in patterns)
        print(
            f"Expected swaps: {total_expected_swaps}, Actual degradations applied: {total_actual_swaps}")
        ...
        if no_change_count > 0:
            print(
                f"WARNING: {no_change_count} degradations had 0 hand coverage!")

        return results

    def apply_pattern_uniform(
            self,
            patterns: list,
    ...
    def apply_pattern_stratified(self, patterns: List[DegradationPattern],
        ...
        """
        out = output_path or self.output_corpus_path.rstrip(
            '/\\') + '_stratified'

        def sampler(
            total,
            size,
            sign_data=None): return self._sample_region_stratified(
            total,
            size,
            n_strata=n_strata,
            sign_data=sign_data)
...
def create_realistic_degradation_patterns(
        num_files) -> List[DegradationPattern]:
    ...
    single_hand = sum(
        1 for p in patterns for t in p.swap_types if t == 'single_hand')
    symmetric = sum(
        1 for p in patterns for t in p.swap_types if t == 'symmetric')
    ...
    print(
        f"  Clean files:    {clean_files}/{num_files} ({100*clean_files/num_files:.0f}%)")
    print(f"  Total swaps:    {total_swaps}")
    print(
        f"  Single-hand:    {single_hand} ({100*single_hand/max(total_swaps,1):.0f}%)")
    print(
        f"  Symmetric:      {symmetric} ({100*symmetric/max(total_swaps,1):.0f}%)")
    ...
    for i, p in enumerate(patterns[:3]):  # Print first 3 patterns
        print(
            f"  Pattern {i}: num_swaps={p.num_swaps}, swap_sizes={p.swap_sizes}, swap_types={p.swap_types}")
...
if __name__ == "__main__":
    gaussian_config = {
        ...
        'means': [0.0897, 0.2921, 0.6762, 0.8856],
    ...
    patterns = create_realistic_degradation_patterns(
        len(degradator.corpus_files))
