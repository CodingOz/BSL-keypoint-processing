...
class SignDegradator:
    ...
    def frameSwap(self, sign_data, hole_size=5, available_area=None):
        ...
        for i in range(start_frame, min(start_frame + hole_size, len(frames))):
            if i < len(frames):
                ...
                # Swapping None→None or []→[] still counts as degradation in metadata
                hands["left"], hands["right"] = hands.get("right"), hands.get("left")
        
        # DEBUG: Log results
        frames_with_data = len(left_hand_frames) + len(right_hand_frames)
        print(f"      frameSwap result: {len(left_hand_frames)} left, {len(right_hand_frames)} right frames with data (expected {hole_size})")
        if frames_with_data < hole_size:
            print(f"      INFO: Only {frames_with_data}/{hole_size} frames had actual hand data, but all {hole_size} were still swapped")
        
        info = {
            'size': hole_size,
            'start_frame': start_frame,
            'frames_affected': max(len(left_hand_frames), len(right_hand_frames)),
    ...
    def singleHandSwap(self, sign_data, hole_size=1, available_area=None, 
        ...
        print(f"      singleHandSwap: available_area={available_area}, hole_size={hole_size}, total_frames={len(frames)}")
        
        if swap_direction is None:
            left_count = 0
            right_count = 0
            for i in range(available_area[0], min(available_area[1], len(frames))):
                hands = frames[i].get("hands", {})
            ...
            if left_count == 0 and right_count == 0:
                swap_direction = np.random.choice(['left_to_right', 'right_to_left'])
            elif left_count == 0:
        ...
        while not valid_hole_found and attempts < max_attempts:
            try:
                start_frame = np.random.randint(
                    available_area[0], 
                    max(available_area[0] + 1, available_area[1] - hole_size + 1)
                )
            except ValueError:
                start_frame = available_area[0]
        ...
        for i in range(start_frame, min(start_frame + hole_size, len(frames))):
            if i < len(frames):
                ...
                # If source has no data, swapping empty→target and None→empty still counts as degradation.
                affected_frames.append(i)
                # Move source hand data to the wrong slot (may be empty list or None)
                hands[target_side] = source_data
                # Source slot becomes empty (hand "disappeared")
                hands[source_side] = []
        
        # DEBUG: Log results
        print(f"        singleHandSwap result: {len(affected_frames)} frames affected (expected {hole_size})")
        if len(affected_frames) < hole_size:
            print(f"        WARNING: Expected {hole_size} affected frames but only got {len(affected_frames)}")
